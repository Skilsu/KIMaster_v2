...
export default createStore({
  ...
  mutations: {
    ...
    SET_DARK_MODE(state, isDarkMode) {
      ...
      localStorage.setItem("isDarkMode", isDarkMode); // Save dark mode state to localStorage
    },
  ...
  actions: {
    ...
    initWebSocket({ commit, state }) {
      ...
      if (currentUrl.startsWith('http://') && currentUrl.includes(':8286')) {
          let modifiedUrl = currentUrl.replace('http://', 'ws://').replace(':8286', ':8210/ws');
          if (modifiedUrl.endsWith('/')) {
              modifiedUrl = modifiedUrl.slice(0, -1);
          }
          socket = new WebSocket(modifiedUrl); // When deployed on Local System
      } else {
          socket = new WebSocket('wss://fk-feedback-test.mni.thm.de/ws'); // Change to 'ws://localhost:8210/ws' when Locally deployed outside of the Docker File
      }
            ...
            switch (receivedJSONObject.response_code) {
              ...
              case 208:
                ...
                if (state.game === ENUMS.games.OTHELLO) {
                  if (
                    receivedJSONObject.moves.includes("64") ||
                    receivedJSONObject.moves.includes(64)
                  ) {
              ...
              case 218: // Currently active Player
                ...
                commit(
                  "setYourTurn",
                  (state.position === "p1" &&
                    receivedJSONObject.cur_player == 1) ||
                    (state.position === "p2" &&
                      receivedJSONObject.cur_player == -1)
                );
                break;
  ...
  getters: {
     ...
     * @param {Object} state - The Vuex state.
     * @returns {boolean} - The dark mode state.
     */
